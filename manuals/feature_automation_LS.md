...
* **REQ-2.1:** Die Datei `adapters/instrument_map.py` (bisher statisches Dictionary) wird abgelöst. Das System liest und schreibt das Mapping fortan aus einer Datei `data/instrument_map.json`.
* **REQ-2.2:** Anpassung in `dev_scripts/momentum_ls_universe.py`: Findet das Skript im eToro Smart Portfolio eine eToro-ID, die nicht in der `instrument_map.json` existiert, darf es das Asset nicht mehr als `null` oder `Unknown` ignorieren.
* **REQ-2.3:** Integration der API-Logik aus `dev_scripts/get_instruments_id.py` in das Universe-Skript: Unbekannte IDs werden über die eToro API abgefragt, um ihr Ticker-Symbol (z.B. `ADA`) zu ermitteln.
* **REQ-2.4:** Das neu gefundene Asset wird in das Format `SYMBOL.ETORO` konvertiert und **automatisch persistent** in die `data/instrument_map.json` geschrieben.

### Phase 3: Automatischer Download fehlender Historien-Daten
**Ziel:** Gewährleistung, dass für das Backtesting-Turnier lückenlose Parquet-Daten aller (auch neu hinzugekommener) Assets vorliegen.

* **REQ-3.1:** Das Master-Skript (oder ein neues dediziertes Modul) liest nach erfolgreichem Abschluss von Phase 2 die generierte Datei `data/universe/momentum_ls.json` ein.
* **REQ-3.2:** Für jedes enthaltene Symbol prüft das System, ob das Verzeichnis `data/nautilus/data/quote_tick/<SYMBOL>/` existiert und nicht leer ist.
* **REQ-3.3:** Wird ein fehlendes Verzeichnis oder Symbol identifiziert, triggert das Master-Skript automatisch den Download via `python3 dev_scripts/momentum_ls_fetch_candles.py --etoro-id <ID> --symbol <SYMBOL> --months 6`.
...
* [ ] Ein neues Asset im eToro Portfolio führt nicht mehr zum Abbruch oder zu einem manuellen Eingriff, sondern wird erkannt, geloggt, heruntergeladen und im Turnier berücksichtigt.
...
* [ ] Alle Änderungen sind rückwärtskompatibel zum bestehenden Nautilus-Framework.
