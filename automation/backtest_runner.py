...
def extract_metrics(engine: BacktestEngine, starting_capital: float, log_fn=None) -> dict:
    ...
    try:
        ...
        profit_factor = (
            min(gross_profit / gross_loss, 10.0) if gross_loss > 0
        ...
        if n < 5:
            ...
            dd_dev = max(math.sqrt(sum(down_sq) / len(down_sq)), 1e-6)
            mean_ret = sum(rets) / n
            sortino = min((mean_ret / dd_dev * math.sqrt(252)), 100.0)
...
def select_winners(
    ...
    if win_counts:
        ...
        aggregate_winner = {
            ...
            "median_sortino": round(get_median(sortinos_by_strat[best]), 4),
            "iqr_sortino": round(get_iqr(sortinos_by_strat[best]), 4),
        }
