...
def test_family_median_mode_without_provided_value_uses_bootstrap_by_default(monkeypatch, tmp_path):
    """Issue #913 Fix 2 — VOR #901 fiel 'family_median' ohne bereitgestelltes
    ...
    #901 machte daraus den ehrlichen, aber unbewertbaren Zustand 'family_median_unavailable'. #913
    führt den tatsächlichen Injektionspfad ein UND eine explizite Kaltstart-Bootstrap-Policy
    (Default 'absolute'): ohne bereitgestellten Wert wird jetzt gegen den absoluten Anker geprüft,
    aber unter der EIGENEN, unterscheidbaren Quelle 'absolute_bootstrap' (nie 'absolute')."""
    import automation.backtest_runner as br
    (tmp_path / "tournament.json").write_text(
    ...
    monkeypatch.setattr(br, "_sortino_numeric_guard_min_periods_cached", False)
    monkeypatch.setattr(br, "_sortino_numeric_guard_reference_mode_cache", None)
    monkeypatch.setattr(br, "_sortino_numeric_guard_reference_bootstrap_cache", None)
    monkeypatch.setattr(br, "config_dir", lambda: tmp_path)

    guard, ref_value, ref_source = _effective_sortino_numeric_guard(25.0, 137)
    assert guard is not None
    assert ref_value == 1600.0
    assert ref_source == "absolute_bootstrap"
    assert ref_source != "absolute"  # die #901-Root-Cause-Behauptung darf nie wieder auftauchen


def test_family_median_mode_defer_bootstrap_is_honest_not_silent(monkeypatch, tmp_path):
    """Issue #913 Fix 2 — 'defer' reproduziert den #901-Zwischenzustand bit-identisch: kein
    bereitgestellter Wert ⇒ (None, None, 'family_median_unavailable'), der Aufrufer muss den Trial
    prunen."""
    import automation.backtest_runner as br
    (tmp_path / "tournament.json").write_text(
        json.dumps({"sortino_numeric_guard_reference": "family_median",
                    "sortino_numeric_guard_reference_bootstrap": "defer",
                    "sortino_numeric_guard_min_periods": 1600}), encoding="utf-8")
    monkeypatch.setattr(br, "_sortino_numeric_guard_min_periods_cache", None)
    monkeypatch.setattr(br, "_sortino_numeric_guard_min_periods_cached", False)
    monkeypatch.setattr(br, "_sortino_numeric_guard_reference_mode_cache", None)
...
def test_coherence_check_fails_under_family_median_with_unavailable_source():
    """Issue #915 — 'family_median_unavailable' ist SEIT #915 EBENFALLS ein FAIL: der Zustand ist
    ehrlich (er behauptet nicht faelschlich, den absoluten Anker verwendet zu haben), aber im
    Produktivbetrieb unzulaessig — er bedeutet 'kein Guard, sortino/psr=None' fuer die betroffenen
    Trials. check_selection_statistic_availability ist die dazugehoerige WIRKUNGS-Invariante."""
    result = inv.check_guard_reference_coherence(
        1600, [130.0], reference_mode="family_median",
...
def test_coherence_check_passes_under_family_median_with_absolute_bootstrap_source():
    """'absolute_bootstrap' (Issue #913 Fix 2 — die Kaltstart-Quelle) ist KEIN Widerspruch (sie
    behauptet nicht faelschlich, die REGULAERE 'absolute'-Referenz sei konfiguriert) und darf den
    Wächter nicht FAILen lassen."""
    result = inv.check_guard_reference_coherence(
        1600, [130.0], reference_mode="family_median",
        observed_guard_reference_sources=["absolute_bootstrap"] * 5)
    assert result.passed is True
