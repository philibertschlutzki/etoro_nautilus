"""Issue #690 — OpeningRangeBreakoutStrategy (SPEC_02, Regime-Roster-Erweiterung).
...
bestehenden Muster in `test_backtest_trades_generated.py`. Verifiziert gegen einen echten
Engine-Lauf: 68 Trades über 60 Tage trend-wechselnder, gap-durchsetzter Kursdaten.
"""
...
def _make_mixed_regime_closes(n_days=60, seed=7):
    """Trend-Wechsel alle 7 Tage + gelegentliche Overnight-Gaps (Tag % 5) — deckt ORB/RSI2/Gap
    gleichermassen ab (Momentum-Ignition am Tagesbeginn, Pullback-Reversion, Gap-Continuation)."""
    import random
    random.seed(seed)
    closes = []
    price = 200.0
...
def test_module_imports_cleanly():
    import automation.strategies.opening_range_breakout  # noqa: F401


def test_registered_in_strategies_json():
    import json
    data = json.loads(Path("automation/config/strategies.json").read_text("utf-8"))
    entries = [s for s in data["strategies"] if s["strategy_class"] == "OpeningRangeBreakoutStrategy"]
    assert len(entries) == 1
    entry = entries[0]
    assert entry["active"] is True
    assert entry["strategy_module"] == "automation.strategies.opening_range_breakout"
    assert entry["config_class"] == "OpeningRangeBreakoutConfig"
...
def test_defaults_present_and_match_spec():
    ...
    d = defaults["OpeningRangeBreakoutStrategy"]
    assert d["or_bars"] == 3
    assert d["or_atr_buffer"] == 0.25
...
def test_spaces_branch_produces_valid_params():
    ...
    for key in ("or_bars", "or_atr_buffer", "cooldown_bars", "atr_period",
                "atr_trailing_multiplier", "max_bars_in_trade"):
        assert key in params
    assert 2 <= params["or_bars"] <= 8
...
def test_backtest_generates_trades(tmp_path):
    ...
    strat = {
        ...
        "strategy_module": "automation.strategies.opening_range_breakout",
        "config_class": "OpeningRangeBreakoutConfig",
    ...
    assert total_trades > 20, f"Erwartet ausreichende ORB-Trade-Frequenz, erhielt {total_trades}"
