...
def test_monotonicity_unit_test():
    ...
    weights["penalty_relative_cap"] = (
        None  # Disable cap to test purely monotonicity of dispersion
    )

    folds_low = [2.0, 4.0]
    m_low = get_base_metrics(oos_sortino=3.0, is_sortino_median=3.0, folds=folds_low)
    reward_low = compute_reward(
        m_low, universe_size=2, weights=weights, risk_dd_cap=0.30
    )
    ...
    m_high = get_base_metrics(
        oos_sortino=13.0, is_sortino_median=13.0, folds=folds_high
    )
    reward_high = compute_reward(
        m_high, universe_size=2, weights=weights, risk_dd_cap=0.30
    )
...
def test_cap_unit_test():
    ...
    assert reward == pytest.approx(
        expected_reward_if_capped, abs=1e-5
    ), f"Reward {reward} not equal to {expected_reward_if_capped}"
