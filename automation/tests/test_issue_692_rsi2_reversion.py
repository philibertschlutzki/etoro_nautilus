"""Issue #692 — Rsi2ReversionStrategy (SPEC_04, Regime-Roster-Erweiterung).
...
echten Engine-Lauf: 59 Trades über 60 Tage trend-wechselnder Kursdaten (höchste Trade-
Frequenz der fünf neuen Strategien, wie in SPEC_04 erwartet).
"""
...
def test_module_imports_cleanly():
    import automation.strategies.rsi2_reversion  # noqa: F401


def test_registered_in_strategies_json():
    import json
    data = json.loads(Path("automation/config/strategies.json").read_text("utf-8"))
    entries = [s for s in data["strategies"] if s["strategy_class"] == "Rsi2ReversionStrategy"]
    assert len(entries) == 1
    entry = entries[0]
    assert entry["active"] is True
    assert entry["strategy_module"] == "automation.strategies.rsi2_reversion"
    assert entry["config_class"] == "Rsi2ReversionConfig"
...
def test_defaults_present_and_match_spec():
    ...
    d = defaults["Rsi2ReversionStrategy"]
    assert d["rsi_period"] == 2
...
def test_spaces_branch_produces_valid_params():
    ...
    trial = study.ask()
    params = sample_params("Rsi2ReversionStrategy", trial)
    for key in ("rsi_period", "rsi_oversold", "rsi_overbought", "ema_period",
                "cooldown_bars", "atr_period", "atr_trailing_multiplier", "max_bars_in_trade"):
        assert key in params
    assert 5.0 <= params["rsi_oversold"] <= 25.0
...
def test_backtest_generates_trades(tmp_path):
    ...
    strat = {
        "strategy_class": "Rsi2ReversionStrategy",
        "strategy_module": "automation.strategies.rsi2_reversion",
        "config_class": "Rsi2ReversionConfig",
        "params": {"allow_short": True, "cooldown_bars": 1},
    }
    ...
    assert total_trades > 20, f"Erwartet hohe RSI2-Pullback-Frequenz, erhielt {total_trades}"
