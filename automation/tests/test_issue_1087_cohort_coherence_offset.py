"""Issue #1087 (Katalog #920) — ``check_report_cohort_coherence`` misst den OFFSET zum
Laufbeginn/-ende, nicht nur die Spannweite der Studies-Vereinigung.
...
Study bleibt UNTER der Spannweiten-Schranke, wird aber durch die neue Offset-Klausel erkannt.
"""
...
def test_span_clause_alone_would_miss_a_leading_foreign_study():
    ...
    span_only = inv.check_report_cohort_coherence(records, wallclock_s=wallclock_s)
    assert span_only.passed is True, (
        "Spannweite allein haette die fremde Study NICHT erkannt — genau die #1087-Root-Cause.")

    # Mit run_started_at_utc (die neuen Offset-Klauseln) erkennt derselbe Report die Kontamination.
    result = inv.check_report_cohort_coherence(
