...
def test_shipped_config_moves_risk_adjusted_gate_to_requires_all():
    ...
    # Fehlerklasse: der Arm war ueber fuenf Laeufe strukturell unerreichbar). Issue #867 — der
    # danach einzig verbliebene Arm (min_profit_factor) zog seinerseits nach eligible_requires_all
    # um (verkleidete Konjunktion, Pitfall #279); eligible_requires_any ist seither bewusst leer.
    assert set(TCFG["eligible_requires_any"]) == set()
    assert "min_profit_factor" in canonical_all
