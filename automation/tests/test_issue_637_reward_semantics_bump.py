...
_EXPECTED_FINGERPRINT_BY_VERSION = {
    ...
    26: _scale_fingerprint(CFG),  # Issue #1248/GH #1118 (oos_min_psr aus eligible_requires_all entfernt, Grenzbeitrag 0.0 gegenueber oos_min_alpha_tstat) + Issue #1250/GH #1120 (oos_min_alpha_tstat_mode-Kalibrierinfrastruktur, Default 'static' bit-identisch) + Issue #1257/GH #1127 (total_return/expectancy_capital_weighted teilen sich seit diesem Fix dieselbe kalibrierte-Slippage-Kostenbasis, backtest_runner._apply_calibrated_slippage_to_mtm_series) -- alle drei sind Eligibility-/Selektionsregel-/Kostenbasis-Bumps auf der TOURNAMENT.JSON-Gate-Zusammensetzung bzw. der Renditeserien-Konstruktion in backtest_runner.py, keiner fuehrt eine neue/geaenderte reward.py-Gewichts-/Skalen-Konstante (_SCALE_KEYS) ein -- Skalen-Konstanten unveraendert.
    27: _scale_fingerprint(CFG),  # Issue #1255/GH #1125 (Pitfall #454-Klasse) -- backtest_runner._evaluate_oos_eligibility konsumiert fuer das oos_min_alpha_tstat-Gate seither den HC3-robusten Schaetzer (oos_alpha_tstat_hc3) statt der klassischen, homoskedastie-unterstellenden SE-Formel; die Eligibilitaets-Entscheidung kann sich dadurch je Trial aendern (t(alpha) klassisch != HC3 im Allgemeinen). Ein Eligibility-/Gate-Statistik-Bump, keine neue/geaenderte reward.py-Gewichts-/Skalen-Konstante (_SCALE_KEYS) -- Skalen-Konstanten unveraendert.
}
