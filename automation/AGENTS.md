...
**Symptom:** In Backtests mit 100% Win Rate generieren bestimmte Metriken mathematische Artefakte (z.B. Profit Factor = 999.00 und Sortino Ratio = 0.00). Später wurden zur Abhilfe harte Caps (z.B. `50.0`, `100.0`) eingesetzt, was jedoch die Aggregat-Mediane bei der Selektion verfälschte, da All-Win-Szenarien fälschlich als exzellente Werte in Mediane einflossen.
**Root Cause:** Fallback bei `gross_loss == 0` oder `dd_dev <= 0` war früher `999.0` und dann gecappt auf `50.0`/`100.0`. Dies tarnt undefinierte finanzmathematische Zustände als extrem gute Performance.
**Fix (Issue #209):** Hardcaps (`MAX_CAP`, `CALMAR_CAP`) wurden restlos entfernt. Undefinierte Metriken (Nenner=0, z.B. bei All-Win) geben nun korrekt und transparent `None` zurück (wird als `null` in JSON serialisiert). Dies verhindert, dass solche Zustände in die Median-Berechnungen einfließen. In der Konsolenausgabe werden sie durch deskriptive Fallbacks wie `"n/a(win)"` visuell klar gekennzeichnet (gesteuert durch das neue Feld `losses_count`).
**Wichtige Architektur-Regel:** Downstream-Systeme und Aggregat-Funktionen (wie `get_median`) müssen stets `None`-Werte explizit herausfiltern, da Metriken auf natürliche Weise durch All-Win-Szenarien undefiniert sein können.
**Betroffen:** `automation/backtest_runner.py`
