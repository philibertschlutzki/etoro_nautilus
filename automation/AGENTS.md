...
|-------|----------|---------|
| 2026-05-31 | **Issue #86 (OOS-Eligibility-Aggregation korrigiert):** Aggregation von OOS-Metriken im `backtest_runner.py` in `select_winners` repariert: Verlierer-Symbole werden ausgefiltert, Ratios (Sortino, Profit-Factor, etc.) werden über den Median (`get_median()`) zusammengefasst und Trades summiert statt gemittelt. Einführung von `oos_min_trades` und `oos_min_total_return` in `tournament.json` für spezifische OOS-Filterung in `_is_eligible()`. | `automation/backtest_runner.py`, `automation/config/tournament.json`, `automation/tests/test_oos_aggregation.py`, `automation/AGENTS.md` |
