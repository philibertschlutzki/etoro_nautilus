...
| 2026-06-07 | **Issue #260 (Fix Breakout Lookback Contamination & Alternation Lock):** Behoben: Invertierte Signale und massives Overtrading durch "Lookback-Contamination" in `DynamicBreakoutStrategy` (History wurde vor Min/Max-Evaluierung aktualisiert). Signal-Status von String-Variable auf dynamische Cache-Prüfung (`positions_open`) umgestellt, um Desynchronisation nach Trailing Stops zu verhindern. State Mutation (Pitfall #41) und saubere Integration in `on_position_closed` sichergestellt. | `automation/strategies/dynamic_breakout.py`, `automation/AGENTS.md` |
