...
3. **Risiko-Ratios (Sortino, PF, max_drawdown):** Werden AB SOFORT NICHT mehr als Median der Pairs berechnet (siehe Issue #286). Der `aggregate_winner` wird stattdessen aus einer zeitlich gemergten, chronologisch sortierten Portfolio-Equity-Kurve (über die in `extract_metrics` gesammelten `_oos_trade_records`) via `_calculate_stats` ermittelt. Die Median-Logik bleibt AUSSCHLIESSLICH als Tie-Breaker für die Pre-Gating-Auswahl auf Symbolebene bestehen, während das finale Aggregat die echte Portfolio-Kurve abbildet.
...
**WICHTIG für Agenten:** Dieser hybride Aggregations-Zustand ist gewollt.
