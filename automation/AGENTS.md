...
**Symptom:** In der Turnierauswertung ergab `win_rate * total_trades` keinen ganzzahligen Wert. Die OOS-Rendite wirkte im Vergleich zu Einzel-Symbolen inkonsistent.
**Root Cause:** Die Aggregation vermischte arithmetische Mittelwerte (für Trades) mit Medians (für Win-Rates). Dies zerstörte die zugrundeliegende mathematische Identität der Einzel-Backtests und erzeugte "Frankenstein-Metriken".
**Fix:** Umstellung auf eine echte "Portfolio-Aggregation". Die `total_trades` und absoluten Gewinne aller Sieger-Symbole werden nun summiert, woraus sich eine echte `portfolio_win_rate` errechnet. Die Rendite ist explizit als Equal-Weight-Portfolio-Rendite (arithmetisches Mittel) definiert und über das Feld `aggregation_basis` im JSON-Event transparent gekennzeichnet.
...
|-------|----------|---------|
| 2026-06-07 | **Issue #255 (Fix OOS Metrics Aggregation Artifacts):** Die Berechnung der OOS-Aggregat-Werte in `select_winners` wurde von Misch-Durchschnitten (Mean/Median) auf eine echte Portfolio-Aggregation umgestellt. `total_trades` ist nun die Summe aller Trades der Gewinner-Symbole, `win_rate` wird aus den absoluten rekonstruierten Gewinnen berechnet. `total_return` wurde als Equal-Weight-Mittelwert deklariert und `aggregation_basis` dem JSON-Event hinzugefügt. Der Test in `test_oos_aggregation.py` wurde auf Summenbildung aktualisiert. | `automation/backtest_runner.py`, `automation/daily_orchestrator.py`, `automation/tests/test_oos_aggregation.py`, `automation/AGENTS.md` |
