...
* **Pitfall #50 (Restriktive Strategiefrequenzen vs. OOS-Gating-Fenster - Issue #289):**
  * **Symptom:** Paare mit hervorragenden In-Sample (IS) Werten fallen im Out-of-Sample (OOS) Gate systematisch wegen Trade-Mangel durch (z. B. oos_min_trades unterschritten).
  * **Root Cause:** Bei kurzen OOS-Fenstern (z. B. 30 Tage) generieren restriktive Strategieparameter (z. B. hohe Deviation-Thresholds) zu wenige Signale. Dies führt zu einem "statistischen Blindspot", da die Strategie OOS gar nicht sinnvoll bewertet werden kann.
  * **Lösung:** Die Signalfrequenz von Indikator-Schwellenwerten (wie `deviation_threshold` bei der VwapExhaustionStrategy) muss stets auf das verwendete OOS-Testfenster kalibriert sein, um die Mindestanforderungen an Trades verlässlich zu erfüllen.

| 2026-06-08 | **Issue #289 (Struktureller OOS-Drop bei restriktiven Strategien & Statistik-Trennung):** Anpassung der VwapExhaustionStrategy-Frequenz (`deviation_threshold` auf `0.008` kalibriert) für Kompatibilität mit dem 30-Tage OOS-Fenster. Implementierung einer klaren Trennung zwischen OOS-nicht-evaluierbaren und OOS-fehlgeschlagenen Paaren im Terminal-Logging und JSON-Output. Pitfall #50 in AGENTS.md hinzugefügt. | `automation/strategies/vwap_exhaustion.py`, `automation/config/strategy_defaults.json`, `automation/backtest_runner.py`, `automation/AGENTS.md` |
