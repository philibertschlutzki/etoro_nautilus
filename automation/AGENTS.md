...
| 2026-06-28 | **Issue #461 (Reward-Inversion bei Constraint Failures):** Harter Band-Clamp in `_constraint_failure_reward` hinzugefügt, sodass die Distanzstrafe den Reward niemals unter die `unevaluable_ceiling` (`-9.75`) drückt. Extrem kleine Ziele (`target < 0.05`) in `_shortfall_distance` werden nun skaliert, um eine quadratische Explosion der Strafe zu verhindern. Invariante der Ordnung (`eligible > near-miss > far-miss ≳ unevaluable_ceiling > unevaluable_shaping_band`) wiederhergestellt. `reward_semantics_version` in `optimizer.json` auf 4 erhöht. Property-Test `test_issue_461_reward_no_inversion.py` ergänzt. Pitfall #86 dokumentiert. | `automation/optimizer/reward.py`, `automation/config/optimizer.json`, `automation/tests/test_issue_461_reward_no_inversion.py`, `automation/AGENTS.md` |
...
**Fix/Regel:** Implementierung eines harten Band-Clamps (`max(unevaluable_ceiling, evaluable_floor - penalty)`). Die Distanzstrafe darf den Reward nun maximal bis auf die `unevaluable_ceiling` drücken. Zusätzlich werden extrem kleine Targets (unter 0.05) bei der Distanzberechnung heraufskaliert.
