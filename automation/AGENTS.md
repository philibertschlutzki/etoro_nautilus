...
| 2026-06-10 | **Issue #339 (Early-Exit Dry-Run Masking & Type Errors):** Fixed Issue #339 (logs_dir function reference TypeError). Added mypy static type checking to pytest-gate.yml to prevent masked early-exit dry-run errors. Documented Pitfall #61. | `automation/backtest_runner.py`, `.github/workflows/pytest-gate.yml`, `automation/AGENTS.md` |
...
### Pitfall #61: Early-Exit Dry-Run Masking & Type Errors (Issue #339)
- **Symptom**: Runtime crashes (like `TypeError` on `os.path.join`) in the main execution loops that are not caught by CI.
- **Root Cause**: The `--dry-run` flag exits the program before the actual worker loop is constructed. Thus, syntax and type errors in the deferred logic are masked.
- **Fix/Architecture Rule**: All Python files in `automation/` must be subjected to static type checking (`mypy`) in the GitHub Actions CI (`pytest-gate.yml`). `mypy` acts as a 100% reliable guard against unexecuted type mismatches (like passing functions instead of strings to `os.path`).
