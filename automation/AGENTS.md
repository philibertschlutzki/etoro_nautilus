...
| 2026-06-04 | **Issue #149 (Fix VwapExhaustionStrategy FIFO vs. Trades):** Behobene Diskrepanz in der `VwapExhaustionStrategy` durch Beenden von Order-Spamming, Implementierung von Signal-Debouncing und Erzwingung der asynchronen Limit-Order-Bereinigung (`_close_position_base`) der Basisklasse vor dem Schließen von Positionen. Pitfall #37 hinzugefügt. | `automation/strategies/vwap_exhaustion.py`, `automation/AGENTS.md` |
| 2026-06-04 | **Issue #170 (Historical Fetcher Logic Bug):** Behoben: Dynamische Berechnung von `real_start_ns` implementiert, wenn `start_ns` <= 0 ist. Behebt einen Bug in `historical_fetcher.py`, bei dem Symbole fälschlicherweise immer als "fehlende Daten" markiert wurden, was zu endlosen, unnötigen API-Aufrufen führte, die dann sofort abgebrochen wurden. | `automation/historical_fetcher.py`, `automation/AGENTS.md` |
