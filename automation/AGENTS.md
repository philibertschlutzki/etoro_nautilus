...
- **`optimizer.json`**:
  ...
  Keys: `n_trials`, `n_startup_trials`, `seed`, `penalty_overfit_weight`, `penalty_dd_weight`, `bonus_coverage_weight`, `penalty_unevaluable_oos`, `sortino_clip_abs`, `unevaluable_shaping_span`, `evaluable_floor_epsilon`, `shaping_trade_target`, `lambda_reg`, `promotion_margin`, `reward_mode`.
...
### 🔴 Pitfall #75 — Per-Symbol-Sweep: konstanter Reward −9.75 (Unevaluable-Floor-Kollaps durch saturiertes Shaping) [DIAGNOSTIZIERT — Fix offen: Issues #404–#410]
...
**Geforderter Fix (Issues #404–#410, sequenziell):** #404 (P0) Per-Symbol-Telemetrie (`make_symbol_objective` emittiert aktuell KEIN `optimizer_trial_completed`); #405 (P0) Evaluierbarkeit von Gewinner-Status entkoppeln (bei `universe_size==1` aus Pro-Symbol-`_oos_eval` beziehen); #406 (P1) `shaping_trade_target` kontextsensitiv; #407 (P1) kontinuierlicher Eligibility-Gradient (Gate-Distanz); #408 (P2) modale Drop-Reason in `user_attrs`/Proposal; #409 (P2) Fail-Loud-Guard bei Floor-Plateau; #410 (P3) Reward-Semantik-Versionierung + Study-Hygiene.
**Invariante (nach Fix):** Im Per-Symbol-Reward gilt `oos_evaluated` ⇔ „das Symbol hat im OOS-Fenster evaluierbare Trades erzeugt", entkoppelt von „das Symbol hat das volle Tournament-Gate bestanden". Reward-WERT bleibt performance-basiert (kein Gate-Flag) → kein Gate-Gaming (Falle 2). Shaping bleibt monoton und nicht-sättigend, `≤ unevaluable_shaping_span`.
**Betroffen (Diagnose):** `automation/optimizer/reward.py`, `automation/optimizer/parsing.py`, `automation/optimizer/run_optimization.py`, `automation/backtest_runner.py`, `automation/config/optimizer.json`
