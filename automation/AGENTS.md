...
| 2026-06-12 | **Issue #371 (Fix ComboTrendVwap Zero Hardcoding):** Hardcodierte Float-Multiplikatoren (0.98/1.02) für den Trend-Filter und das Literal `24` für das BB-Touch-Window aus `ComboTrendVwapStrategy` entfernt. Diese wurden als optimierbare Variablen `trend_tolerance_pct` (Default: 0.02) und `bb_touch_window` (Default: 24) in die Config extrahiert und in `spaces.py` für Optuna freigegeben. Dies ermöglicht dem Optimierer direkte Kontrolle über die Entry-Frequenz. | `automation/strategies/tesla_combo_strategy.py`, `automation/config/strategy_defaults.json`, `automation/optimizer/spaces.py`, `automation/AGENTS.md` |
...
| 2026-06-12 | **Issue #370 (IS-Activity Metrics in Optimizer Trials):** Added `is_total_trades` and `is_max_trades` metrics to `TournamentMetrics` via `automation/optimizer/parsing.py`. These metrics parse the total and max sum of trades out of the `full_results` array. Included these metrics (alongside `win_count`) in the execution event payload generated by `make_objective` in `automation/optimizer/run_optimization.py`. Dies ermöglicht die klare Unterscheidung zwischen echten OOS-Misserfolgen und mangelnder IS-Aktivität/Eligibility (Frequenzproblem). Bestehende TDD-Tests in `automation/tests/` wurden aktualisiert. | `automation/optimizer/parsing.py`, `automation/optimizer/run_optimization.py`, `automation/tests/test_reward_shaping.py`, `automation/tests/test_parsing_null_winner.py`, `automation/AGENTS.md` |
