...
* **Pitfall #37 (Orphaned Limit Orders via Custom Exits):** Das Überschreiben von Exits in Child-Klassen ohne Beachtung von `orders_open` und dem Async-State der Base-Class führt zu Order-Spamming, Orphaned Limit-Orders und extrem asymmetrischen FIFO-Legs. Strategien dürfen `_close_position` nicht überschreiben, sondern müssen `self._close_position_base()` aus der Basisklasse nutzen, um den korrekten asynchronen Bereinigungsfluss beizubehalten. (Referenz: Issue #149, #290).
* **Pitfall #46 (Observability & Hidden Gates):**
  ...
  * **Single Source of Truth Constraint:** `span_tolerance_days` aus `automation/config/backtest.json` ist die alleinige, anwendungsübergreifende Source of Truth für Datenspannen-Defizite. Jegliche lokale Überschreibung oder unsaubere Fallback-Zuweisung in den Logging-Blöcken (z.B. Fallback auf 1.0 in `backtest_runner.py`) ist strikt untersagt, um Inkonsistenzen zwischen Log und Gating zu verhindern.
...
* **Pitfall #50 (Zero-Trade Statistical Cascades / Micro-Sizing Illusion):**
  Wenn ein Symbol-Strategie-Paar im Backtest 0 Trades generiert, dürfen nachgelagerte Trade-Metrik-Guards (wie der Median-Notional-Floor von $10) nicht evaluiert werden. Da eine leere Liste einen Median von `0.0` impliziert, führt dies ohne `n_trades > 0` Guard zu falschen "Micro-Sizing"-Ablehnungsgründen, die das eigentliche Problem (keine Signal-Auslösung) im Log verschleiern.
* **Pitfall #50 (Gate-Scope vs. Deployment-Scope Mismatch):**
  * Ein notorischer Einzel-Verlierer könnte live geschaltet werden, weil er innerhalb seines isolierten Symbols als lokaler Gewinner hervorging, obwohl seine Strategie das OOS-Gate nie bestanden hat. Dies wird nun in `_build_bots_config` über einen `OOS-DEPLOY-REJECT`-Filter hart blockiert. JSON-Events und Logausgaben im Orchestrator unterscheiden strikt zwischen `median_is_sortino` und `aggregate_oos_sortino`.

### 🟢 Pitfall #50 — Restriktive Strategiefrequenzen vs. OOS-Gating-Fenster (Issue #289)
...
* **Pitfall #50 — Alternation Lock & Cooldown Bypass Trap in Mean Reversion/Breakout:**
  ...
  * **Root Cause:** Ein Logik-Fehler im `can_signal` Guard (ein `or`-Bypass) hebelte in der Keltner-Strategie den Cooldown nach Exits aus. Die `HourlyMeanReversionStrategy` implementierte zudem kein `on_position_closed`, wodurch der State `current_signal` einfror und künstlich eine Alternierung erzwang. Bei der Breakout-Strategie setzte `on_position_closed` den Cooldown-Zähler falsch zurück, sodass sofort wieder eingestiegen werden konnte.
  * **Lösung:** Cooldowns als harte AND-Bedingungen umgesetzt. `on_position_closed` in allen Klassen synchronisiert, sodass `bars_since_last_signal` auf 0 zurückgesetzt wird und ein echter Cooldown nach Stop-Outs/Exits erzwungen wird.
* **🟢 Issue #286 — OOS-Portfolio-max_drawdown Portfolio-Equity-Aggregation**
  ...
  * **Lösung:** Behoben durch die Einführung chronologischer Trade-PnL-Merges ('_oos_trade_records') vor der statistischen Evaluierung. Das Phase-5-Gate in 'daily_orchestrator.py' bleibt unverändert, da die originalen Key-Namen beibehalten wurden, nun aber die echte Portfolio-Equity-Kurve repräsentieren.
