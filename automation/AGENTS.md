...
| 2026-06-09 | **Issue #288 (Tournament Sample-Size Shrinkage & Metric Capping):** Harmonisierung der Capping-Policy für Profit Factor, Sortino Ratio und Calmar Ratio auf strikt `50.0` in `_calculate_stats` eingeführt, um division-by-zero Ausreißer abzufangen. Einführung eines logistischen "Sample-Size Shrinkage" Faktors (`N / (N + k_shrinkage)`) in `select_winners` **auf den finalen Score**, um Low-Sample "Glückstreffer" im Tournament abzuwerten. Dynamisches Sentinel-Handling für "All-Win" Strategien mit `None` Risk-Metriken (`min(scaled, max(organic_ratios))`) statt blindem 50.0 Cap, um organische Ratios nicht zu degradieren. `k_shrinkage` wurde konfigurierbar in `tournament.json` ausgelagert. **Constraint:** Zukünftige Agent-Runs dürfen diese mathematischen Shrinkage- und Capping-Gatekeeper unter keinen Umständen entfernen oder überschreiben. | `automation/backtest_runner.py`, `automation/config/tournament.json`, `automation/AGENTS.md` |
