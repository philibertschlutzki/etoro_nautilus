...
| 2026-06-08 | **Issue #204 (Fix Strategy Clone Bias):** In `automation/config/strategy_defaults.json`, the parameters for `MeanReversionStrategy` were incorrectly identical to `HourlyMeanReversionStrategy` (10/10/1.5), overriding the class defaults (20/20/2.0) and creating a clone bias in tournament selection. Restored `MeanReversionStrategy` to 20/20/2.0 to ensure differentiation between wide and tight Keltner band strategies. Decided against deleting the override entirely to keep configurations explicit. Verified differentiation via `test_strategy_duplication.py`. | `automation/config/strategy_defaults.json`, `automation/AGENTS.md` |
