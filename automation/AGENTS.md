...
### 🟢 Pitfall #93 — oos_covered Lower-Bound-only (Issue #475 / #491)
...
- **Time Series Invariants:** Annualization factors MUST be derived empirically via `31_557_600.0 / median_dt_seconds`. Static factors (e.g., 252) are explicitly deprecated unless required for forced config overrides. **Präzisierung (Review PR #542):** Der Median-Δt trifft die dominante **Intra-Session-Bar-Frequenz** (1h ⇒ 8766) und ist damit gap-robust, **differenziert die Asset-Klasse aber NICHT** — Equity-1h (mit Overnight-/Wochenend-Lücken) und 24/7-Krypto-1h ergeben *beide* 8766 (uniforme Trading-Time-Konvention). Innerhalb *einer* Asset-Klasse ist das ein konstanter `√factor`-Multiplikator und damit **ranking-neutral** für den TPE-Sampler; die **absoluten** Sortino-Gates (`oos_min_sortino`) sowie echte Cross-Asset-Vergleiche (Equity vs. Krypto) benötigen für Marktstunden-Korrektheit eine **Observations-per-Year-Skalierung** (`n_periods · 31_557_600 / total_span_seconds`) — das wird vom Median **NICHT** implizit gelöst. **Entscheidung (PR #542):** 8766 ist der akzeptierte, uniforme 1h-Standard (Trading-Time-Konvention, ranking-neutral); die Cross-Asset-Korrektheit der **absoluten** Gates ist als Folge-Issue **#543** (Observations-per-Year, inkl. Gap-Stresstests) getrackt. *(Umsetzung: `backtest_runner.py::_get_annualization_factor`; 1h-Bars ⇒ Faktor 8766 ≫ 252; Issue #532.)*
...
### 🟢 Pitfall #116 — Per-Symbol-Sweep ohne `global_best`: Shrinkage & Warm-Start silent inaktiv [BEHOBEN: GH-#564, #579]
...
**Fix/Regel:** Fehlt `global_best` im Per-Symbol-Pfad ⇒ **LAUT** warnen (Study-Attr `shrinkage_inactive=true`) UND auf `strategy_defaults.json` als Shrinkage-Referenz + Warm-Start-Seed zurückfallen (`resolve_symbol_shrinkage_seed`). Ein `global_best` Parameter-Vektor muss zwingend mindestens eine aktive tunable Dimension (aus `bounds.extract_numeric_bounds`) beinhalten, um als valider Seed akzeptiert zu werden. Reine Struktur-Flags (z. B. `allow_short`) genügen nicht und lösen den Fallback-Pfad aus. `param_pen` zieht dann Richtung Default (der ökonomisch begründete Prior) statt ins Leere — es darf **nie still 0** werden.
**Betroffen:** `automation/optimizer/run_optimization.py`
