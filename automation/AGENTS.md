...
| 2026-06-09 | **Issue #288 (Tournament Sample-Size Shrinkage & Metric Capping):** Harmonisierung der Capping-Policy für Profit Factor, Sortino Ratio und Calmar Ratio auf strikt `50.0` in `_calculate_stats` eingeführt, um division-by-zero Ausreißer abzufangen. Einführung eines logistischen "Sample-Size Shrinkage" Faktors (`N / (N + 20)`) in `select_winners`, um Low-Sample "Glückstreffer" im Tournament abzuwerten. Dynamisches Sentinel-Handling für "All-Win" Strategien mit `None` Risk-Metriken (`min(50.0, max(2.0, 50.0 * (n_trades / 50.0)))`) statt blindem 50.0 Cap. **Constraint:** Zukünftige Agent-Runs dürfen diese mathematischen Shrinkage- und Capping-Gatekeeper unter keinen Umständen entfernen oder überschreiben. | `automation/backtest_runner.py`, `automation/AGENTS.md` |
