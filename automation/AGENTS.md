...
* **Pitfall #50 (Gate-Scope vs. Deployment-Scope Mismatch):**
  * Ein notorischer Einzel-Verlierer könnte live geschaltet werden, weil er innerhalb seines isolierten Symbols als lokaler Gewinner hervorging, obwohl seine Strategie das OOS-Gate nie bestanden hat. Dies wird nun in `_build_bots_config` über einen `OOS-DEPLOY-REJECT`-Filter hart blockiert.
