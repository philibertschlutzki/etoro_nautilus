...
### 🟢 Pitfall #91 — OOS-Rejection-Taxonomie (Issue #493)
**Symptom:** Eine Trial hat das OOS-Fenster abgedeckt (`oos_covered=True`) und auch tatsächlich getradet (`oos_total_trades > 0`), aber wurde vom IS-Gate verworfen (`is_eligible=False` ⇒ `oos_evaluated=False`). Sie wird fälschlich als `REJECT_OOS_INACTIVE` klassifiziert, was mangelnde OOS-Trades suggeriert.
**Fix:** `_classify_is_rejection_detail` nutzt nun `oos_total_trades > 0` (via Subprozess-Metadaten) als Weiche, um diese Trials als `REJECT_OOS_DISCARDED_BY_IS_GATE` zu klassifizieren, sodass echtes "Inactive" vom "IS-Discard" unterschieden wird.
