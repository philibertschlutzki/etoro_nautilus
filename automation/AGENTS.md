...
| 2026-06-13 | **ComboTrend VWAP Rollierendes Fenster Update (Issue #372):** Die `ComboTrendVwapStrategy` wurde überarbeitet, um anstelle einer unendlichen Akkumulierung des VWAP ein rollierendes VWAP-Fenster (`vwap_period`) analog zur `VwapExhaustionStrategy` zu nutzen. Diese Änderung stellt sicher, dass das VWAP pfadunabhängig berechnet wird und nun für den Optimizer durch Aufnahme in `spaces.py` und `strategy_defaults.json` parametrisierbar ist. | `automation/strategies/tesla_combo_strategy.py`, `automation/config/strategy_defaults.json`, `automation/optimizer/spaces.py`, `automation/AGENTS.md` |
