...
* **🟢 Behoben: Issue #286 — Portfolio-Equity-Aggregation für den aggregate_winner:**
  * **Problem:** `aggregate_winner` in `backtest_runner.py:select_winners` verwendete den Median der Metriken der Einzel-Paare, was den wahren Drawdown drastisch unterschätzte.
  * **Lösung:** OOS-Trade-Records aus Worker-Läufen extrahiert. Diese werden für die Best-Strategie auf Portfolio-Ebene aggregiert, chronologisch sortiert und erst dann via `_calculate_stats` ausgewertet. Resultat: Echte Portfolio-Equity-Curve Metriken.
...
| 2026-06-08 | **Issue #286 (Portfolio-Equity-Aggregation für den aggregate_winner):** OOS-Metriken (Max Drawdown, Sortino, Profit Factor, etc.) für den Gesamtsieger (`aggregate_winner`) in `backtest_runner.py:select_winners` werden jetzt durch chronologisches Zusammenführen aller OOS-Trades der Einzel-Gewinner in ein Gesamt-Portfolio und anschließender Evaluierung berechnet (Portfolio Equity Curve), anstatt fälschlicherweise den Median der OOS-Metriken pro Pair zu nehmen. | `automation/backtest_runner.py`, `automation/tests/test_oos_aggregation.py`, `automation/AGENTS.md` |
