...
| 2026-06-06 | **Issue #229 (Fix Strategies Flat-Lock & Under-Trading):** Root Cause für Flat-Lock auf 2 Trades behoben. State Mutations (`current_signal`, `bars_since_last_signal`) vor Early-Returns in `mean_reversion.py`, `hourly_mean_reversion.py` und `tesla_combo_strategy.py` verlagert, um Blockaden nach abgelehnten Entries zu verhindern (Pitfall #41). Übersteuerte Parameter der `ComboTrendVwapStrategy` rekalibriert (`cooldown_bars=4`, `bb_entry_tolerance=24`). Warmup-Freigabe des `trend_filter_sma` in `hourly_strategy_base.py` überarbeitet und Missing State-Resets bei `on_position_closed`, `on_order_rejected` und `on_order_canceled` hinzugefügt, um Async-Locks zu verhindern. Double-Feeding des `trend_filter_sma` aus `can_go_long` entfernt. | `automation/strategies/mean_reversion.py`, `automation/strategies/hourly_mean_reversion.py`, `automation/strategies/tesla_combo_strategy.py`, `automation/strategies/hourly_strategy_base.py`, `automation/config/strategy_defaults.json`, `automation/tests/test_issue_229.py`, `automation/AGENTS.md` |
