...
**Symptom:** `FlashCrashReversalStrategy` zeigte PBO=0.94 bei nur 36 Configs — die CSCV-Split-Metrik war der rohe Gruppen-MITTELWERT der per-Bar-MtM-Return-Serie (`oos_period_returns = mtm_series.pct_change()`), die zwischen Trades (Config in Cash) von Nullen dominiert wird.
