...
* **Pitfall #47 (Base-Exit Signal State Lock / Silent Trade Stagnation):** Wenn Positionen durch automatisierte Exits der Basisklasse ('HourlyStrategyBase') geschlossen werden, verbleibt das lokale 'self.current_signal' der Subklasse im alten Zustand ("BUY"/"SELL") und blockiert Folge-Entries dauerhaft, sofern es nicht via 'on_position_closed' explizit auf 'None' zurückgesetzt wird. *Jede Subklasse, die lokale Signal-Zustände (wie `current_signal`) führt, MUSS `on_position_closed` überschreiben, den `super()`-Call absetzen und den eigenen State bereinigen, um asynchrone Exits der Basisklasse aufzufangen.* Dies grenzt sich von Pitfall #17 ab, welches Flat-Locks nach *manuellen* Reverse-Entries betrifft.
