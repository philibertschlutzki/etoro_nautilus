...
### 🟡 Pitfall #75 — Per-Symbol-Sweep: Unevaluable-Floor-Kollaps [TEILWEISE — konstantes −9.75-Plateau behoben (#404–#410); empirisch (2026-06-24) weiterhin 0 evaluable Trials → Issue #413]
...
- ⚠️ **#409 (P2) Fail-Loud-Guard — INEFFEKTIV im v3-Shaping-Regime (→ #413):** `floor_plateau_callback` prüft `all(abs(value − (−9.75)) < 1e-6)`. Da #406/#407 unevaluable Trials *absichtlich* unter −9.75 verteilen (Gradient), ist dieses Wert-Gleichheits-Prädikat im geshapeten Regime strukturell unerfüllbar — der Guard feuert in genau dem Fall nie, für den er gebaut wurde. Der `test_issue_409`-Test füttert ausschließlich exakt −9.75 und maskiert die Blindheit. Ersatz: evaluable-basierter Guard (`all(trial.user_attr['oos_evaluated'] is False)`), Issue #413.
...
### 🔴 Pitfall #76 — Optuna/SQLite `create_all`-DDL-Race bei parallelen Per-Symbol-Studies (Issue #411)
...
**Fix:** (1) `_preinit_study_storage(study_name)` serialisiert im Hauptthread vor dem `executor.map` für jeden eindeutigen `study_name` (erzwingt `create_all` einmal → Worker treffen „exists"); (2) defensiver Retry in `optimize_symbol` auf `… already exists`-`OperationalError`. Test: ≥8 Threads gegen eine frische `tmp_path`-DB ⇒ kein Crash, genau eine Study.

### 🔴 Pitfall #77 — Doppelte `(strategy, symbol)`-Paare kollabieren Worker auf eine Study (Issue #412)
...
**Fix:** order-preserving Dedup in `load_symbol_universe` (`dict.fromkeys`) UND `enumerate_tunable_pairs`; harte Fail-Fast-Assertion in `run_per_symbol_sweep`, falls zwei Paare denselben `study_name` ergäben. Daten-Hygiene: `data/universe/momentum_ls.json` auf Duplikate prüfen. **Eindeutigkeit ist eine harte Vorbedingung der Per-Study-Parallelität, kein Implementierungsdetail.**

### 🟡 Pitfall #78 — Sweep-Entrypoint ohne Logging-Init ⇒ `[JSON_EVENT]`-Telemetrie verworfen (Issue #414)
...
**Fix:** `setup_bot_logging("optimizer")` als erste Anweisung in `sweep.main()` (File-Handler DEBUG → JSONL, Stream INFO, `propagate=False`; kollidiert nicht mit Optunas Logger). Per-Trial-Events in die Datei, Konsole für Header/WARNINGs/Timing-Summaries. **Vorbedingung für die Wurzeldiagnose von #413.**

### 🟡 Pitfall #79 — Fehlende Backtest-Zeitdauer & verschluckter Subprozess-Output (Issues #415/#416)
...
**Fix:** Wall-Clock in `run_backtest` (beide Modi); `backtest_ms` ins Per-Trial-Event; neue Events `optimizer_study_completed` (n_trials, evaluable_trials, best_value, backtest_ms_total/median, wallclock_s) und `sweep_completed` (wallclock_s) + menschenlesbare Konsolen-Schlusszeile; Subprozess-stdout/stderr pro Trial nach `trial_dir/logs/` persistieren (auch im Erfolgsfall).
...
*Zuletzt aktualisiert: 2026-06-23. Datum und Changelog bei jeder Änderung an dieser Datei aktualisieren.*
