...
* **Hinweis (Issue #179):** Die eToro API liefert beim Endpoint `instruments` derzeit oftmals **keine Precision-Felder für reine Equities**. Dies ist ein bekanntes Limit der API, weshalb das fehlende Feld für Equities kein Fehler ist. Der in `_fallback_precisions` verwendete Standard-Fallback auf `(2,2)` ist funktional korrekt und fängt dieses Verhalten sicher ab.
...
### Pitfall #37: Profit Factor / Sortino NoneType Artefakte bei Zero-Loss (Issue #150 / #209)
**Symptom:** In Backtests mit 100% Win Rate generieren bestimmte Metriken mathematische Artefakte (z.B. Sortino Ratio = 50.0 oder Profit Factor = 50.0). Dies verzerrt die Aggregat-Mediane und Auswertung extrem, wenn die Werte durch künstliche Caps (`MAX_CAP = 50.0`, `CALMAR_CAP = 100.0`) künstlich hoch gehalten werden.
**Root Cause:** Fallback bei undefinierten Nennern (z.B. `gross_loss == 0`) waren hardcodierte `MAX_CAP`-Werte, die die Scores nach oben verzerrten und echte Resultate verfälschten.
**Fix:** Die harten Caps wurden vollständig entfernt (Issue #209). Undefinierte finanzmathematische Zustände (All-Win-Szenarien ohne Verluste) werden nun korrekt und konsequent mit `None` abgebildet. Dies verhindert Median-Verfälschungen bei der Aggregation. Im CLI-Output rendern diese Werte distinkt als `n/a(win)` oder `n/a(<min)`. Eine dedizierte Filter-Gating-Logik in `_is_eligible` wirft diese Kandidaten proaktiv ab.
...
|-------|----------|---------|
| 2026-06-05 | **Issue #209 (Zero-Loss Cap Bug):** Hardcaps (50.0, 100.0) für Sortino/Calmar/PF in `_calculate_stats` entfernt. All-Win-Szenarien geben nun korrekt `None` zurück und rendern im Terminal distinkt (z.B. `n/a(win)`), damit Aggregat-Mediane nicht mehr durch künstliche Score-Artefakte verzerrt werden. `losses_count` ins Output-Dict integriert. Rang-Normalisierung abgesichert, um None als 0.0 zu behandeln. | `automation/backtest_runner.py`, `automation/AGENTS.md` |
...
| 2026-06-06 | **Issue #194 (Sizing Bug & Impossible Risk Metrics Guards):** Fixed sizing calculation bug causing microscopic returns by strictly quantizing quantity to align with tick precision and size_increment in `HourlyStrategyBase._compute_quantity`. Introduced `EPSILON = 1e-9` in risk metric denominators (Profit Factor, Sortino, Calmar) and clamped extreme values to `MAX_CAP = 50.0` (100.0 for Calmar). Enforced sample size minimums for near-all-win situations (`losses_count < 2 and n < 50`) to return `None` in `backtest_runner.py._calculate_stats`. Added coverage tests for these bounds. | `automation/strategies/hourly_strategy_base.py`, `automation/backtest_runner.py`, `automation/tests/test_backtest_runner.py`, `automation/AGENTS.md` |
| 2026-06-07 | **Issue #209 (Zero-Loss Cap Bug):** Hardcaps (50.0, 100.0) für Sortino/Calmar/PF in `_calculate_stats` entfernt. All-Win-Szenarien geben nun korrekt `None` zurück und rendern distinkt (z.B. `n/a(win)`), damit Aggregat-Mediane nicht mehr durch künstliche Score-Artefakte verzerrt werden. `losses_count` ins Output-Dict integriert. | `automation/backtest_runner.py`, `automation/tests/test_backtest_runner.py`, `automation/AGENTS.md` |
