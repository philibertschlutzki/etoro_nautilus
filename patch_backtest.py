...
content = re.sub(r'import sys\nimport os\n\n(import argparse)', f"{new_sys_path}\n\\1", content, count=1)

# Ensure no adapters imports
content = re.sub(r'from adapters\..+? import .+\n', '', content)
content = content.replace("from automation.config.strategies import", "# ")

# Check what was imported from adapters, usually it's get_size_precision, etc. We will add the required imports instead.
new_imports = """
import pyarrow.parquet as pq
from automation.utils import _fallback_precisions
import importlib

def read_precisions_from_parquet(parquet_path: Path | str, instrument_id: str = None) -> tuple[int, int]:
    try:
...
content = content.replace("from nautilus_trader.core.datetime import dt_to_unix_nanos", "from nautilus_trader.core.datetime import dt_to_unix_nanos" + new_imports)
